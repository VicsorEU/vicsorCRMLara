            ...
            <div>
                ...
                <select name="parent_id" class="w-full rounded-xl border px-3 py-2">
                    ...
                    @foreach($parents as $p)
                        <option
                            value="{{ $p->id }}" @selected(old('parent_id', $category->parent_id)==$p->id)>{{ $p->name }}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div class="space-y-4">
            <div>
                ...
                <div class="flex items-start gap-4">
                    ...
                    <template x-if="!previewUrl">
                        <img x-show="currentUrl" :src="currentUrl" class="h-28 w-28 object-cover rounded-xl border"
                             alt="current">
                    </template>
                    <div class="flex-1 space-y-2">
                        ...
                        <div class="flex gap-2">
                            <button type="button" class="px-3 py-1.5 rounded-lg border"
                                    @click="clearFile()">Очистить выбор
                            </button>
            ...
            <div>
                ...
                <textarea name="description" rows="5"
                          class="w-full rounded-xl border px-3 py-2">{{ old('description', $category->description) }}</textarea>
            </div>
        </div>
    </div>

    <div class="flex gap-2">
        <x-ui.button type="button" id="categoryBtnForm" @click="submitForm">Сохранить</x-ui.button>
                ...
                fetch(form.action, {
                    ...
                    .then(response => {
                        if (response.success) {
                            ...
                            if (form.dataset.mode === 'create' && response.category) {
                            ...
                            if (form.dataset.mode === 'edit' && response.category) {
