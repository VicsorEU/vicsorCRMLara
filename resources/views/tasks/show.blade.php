...
@section('head')
    <style>
        ...
        /* обёртка вокруг main, чтобы оверлей позиционировался в её пределах */
        .lock-scope{ position: relative; }

        /* «уловитель» кликов — только над содержимым .lock-scope */
        .lock-catcher{
            position: absolute; /* ВАЖНО: было fixed */
            inset: 0;
            z-index: 10;
            cursor: not-allowed;
        }

        /* попап по центру той же области */
        .lock-pop{
            position: absolute; /* ВАЖНО: было fixed */
            top: 50%; left: 50%;
...
@section('content')
    <div class="relative">
        <script>
            function lockPopupCenter(message = 'Страница заблокирована для редактирования') {
                return {
                    visible: false,
                    msg: message,
                    t: null,
                    show(){
                        this.visible = true;
                        clearTimeout(this.t);
                        this.t = setTimeout(() => this.visible = false, 1200);
                    }
                }
            }
        </script>
...
<button type="button" class="text-red-600 hover:underline file-del" data-id="${f.id}">Удалить</button>`;
        ...
        <div
            x-data="lockPopupCenter('Задача выполнена. Редактирование недоступно')"
            x-show="$store.task.done"
            x-cloak
            class="lock-catcher"
            @click="show()"
            aria-hidden="true"
        >
            <div class="lock-pop" :data-show="visible ? 1 : 0" x-text="msg"></div>
        </div>
    </div>
@endsection
