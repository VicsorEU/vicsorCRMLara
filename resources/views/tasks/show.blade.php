    ...
    <div class="space-y-4">
        ...
        <form method="post" action="{{ route('tasks.update',$task) }}" class="bg-white border rounded-2xl shadow-soft">
            ...
            <div class="p-5 grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="md:col-span-2">
                    ...
                    <input name="title" class="w-full border rounded-lg px-3 py-2" value="{{ old('title',$task->title) }}">
                </div>
                ...
                <div>
                    ...
                    <select name="type" class="w-full border rounded-lg px-3 py-2">
                        @foreach(['common'=>'Обычная','in'=>'Приход','out'=>'Расход','transfer'=>'Перемещение','adjust'=>'Корректировка'] as $v=>$t)
                            <option value="{{ $v }}" @selected($task->type===$v)>{{ $t }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="priority" class="w-full border rounded-lg px-3 py-2">
                        @foreach(['normal'=>'Обычная','high'=>'Высокая','p1'=>'Высокая (P1)','p2'=>'Критическая (P2)','low'=>'Низкая'] as $v=>$t)
                            <option value="{{ $v }}" @selected($task->priority===$v)>{{ $t }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="md:col-span-3 flex items-center gap-2">
                    <button formaction="{{ route('kanban.timer.start',$task) }}" class="px-3 py-2 rounded-lg border">▶ Старт таймера</button>
                    <button formaction="{{ route('kanban.timer.stop',$task) }}"  class="px-3 py-2 rounded-lg border">■ Стоп</button>
                    <div class="ms-auto text-sm text-slate-600">
                        Всего по задаче:
                        @php $sec = $task->total_seconds; @endphp
                        <strong>{{ sprintf('%02d:%02d:%02d', floor($sec/3600), floor(($sec%3600)/60), $sec%60) }}</strong>
                    </div>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div class="bg-white border rounded-2xl shadow-soft">
                ...
                <div class="p-5">
                    <form class="flex gap-2" method="post" enctype="multipart/form-data" action="{{ route('tasks.files.store',$task) }}">
                        @csrf
                        <input type="file" name="file" class="flex-1 border rounded-lg px-3 py-2" required>
                        <button class="px-3 py-2 rounded-lg border">Загрузить</button>
                    </form>
                    <ul class="mt-3 space-y-2">
                        @forelse($task->files as $f)
                            <li class="flex items-center justify-between">
                                <a class="text-brand-600 hover:underline" href="{{ $f->url }}" target="_blank">{{ $f->original_name }}</a>
            ...
            <div class="bg-white border rounded-2xl shadow-soft">
                ...
                <div class="p-5">
                    <form class="flex gap-2 mb-3" method="post" action="{{ route('tasks.comments.store',$task) }}">
                        ...
                        <button class="px-3 py-2 rounded-lg border">Отправить</button>
                    </form>
                    <div class="space-y-3">
                        @forelse($task->comments as $c)
                            <div>
                                <div class="text-xs text-slate-500">{{ $c->created_at->format('d.m.Y H:i') }} — {{ $c->user->name ?? 'Пользователь #'.$c->user_id }}</div>
                                <div>{{ $c->body }}</div>
                            </div>
                        @empty
        ...
        <div class="bg-white border rounded-2xl shadow-soft">
            ...
            <div class="p-5 overflow-x-auto">
                <table class="min-w-full text-sm">
                    ...
                    <tr><th class="py-2 pr-4">Пользователь</th><th class="py-2 pr-4">Начало</th><th class="py-2 pr-4">Конец</th><th class="py-2">Длительность</th></tr>
                    </thead>
                    <tbody>
                    @foreach($task->timers as $t)
                        <tr class="border-t">
                            ...
                            <td class="py-2">
                                @php $d=$t->duration_sec; @endphp
                                {{ $d ? sprintf('%02d:%02d:%02d', floor($d/3600), floor(($d%3600)/60), $d%60) : 'идёт...' }}
                            </td>
                        </tr>
                    @endforeach
                    </tbody>
                ...
                <form class="mt-4 flex flex-wrap items-end gap-2"
                    ...
                    <button class="px-3 py-2 rounded-lg border">Добавить</button>
                </form>
            </div>
        </div>
    </div>
